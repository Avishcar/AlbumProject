...
const AlbumVertical = ({ data,bigcardClick }) => {
    const navigation = useNavigation()
    return (
        <TouchableOpacity style={{margin:10}} onPress={()=>bigcardClick(data.id)}>
            <Image
        ...
        />
          <Text style={{flex: 1, flexWrap:"nowrap" ,width:100,fontSize:16,fontWeight:"bold",color:"#000000",flexWrap:"nowrap"}} numberOfLines={1}>{data.title}</Text>
          <Text>25</Text>
        </TouchableOpacity>
    )
